...
const steps = [
  {
    icon: <UserPlus className="h-10 w-10 text-topmate-600" />,
  ...
  {
    icon: <Search className="h-10 w-10 text-topmate-600" />,
  ...
  {
    icon: <Calendar className="h-10 w-10 text-topmate-600" />,
    ...
    <section id="how-it-works" className="py-24 md:py-32 relative bg-white">
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-16 animate-on-scroll">
          <h2 className="text-3xl md:text-5xl font-bold mb-6 figma-heading">How KnockMe Works</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {steps.map((step, index) => (
            <div key={index} className="animate-on-scroll" style={{ animationDelay: `${index * 0.2}s` }}>
              <Card className="border-0 figma-card h-full shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden bg-white">
                <CardContent className="p-8 flex flex-col items-center text-center">
                  <div className="feature-icon-container mb-6 relative">
                    {step.icon}
